import { CREATE_CLIENT_ERROR, CREATE_CLIENT_LOADING, CREATE_CLIENT_SUCCESS, CREATE_CLIENT_RESET } from './constants';

const initialState = {
  loading: false,
  success: false,
  error: null,
  data: null,
  message: null
};

const createClientReducer = (state = initialState, action) => {
  const { type, payload } = action;
  switch (type) {
    case CREATE_CLIENT_LOADING:
      return { ...state, loading: true };
    case CREATE_CLIENT_SUCCESS:
      return {
        ...state,
        loading: false,
        success: payload?.success,
        error: null,
        data: payload.data,
        message: payload.message
      };
    case CREATE_CLIENT_ERROR:
      return { ...state, loading: false, error: payload.error, message: payload.message };
    case CREATE_CLIENT_RESET:
      return initialState;
    default:
      return state;
  }
};

export default createClientReducer;
